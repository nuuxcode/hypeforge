...
Each compliment is generated by a distinct comedic persona in its own model call. The app picks one Grand, one Mythic, and one Chaotic voice per run; every persona owns an explicit imagery lane and carries one few-shot example written for a person who never appears in real input. The three primary pipelines run concurrently. If one slot fails, it gets one same-bucket fallback; success is atomic, so the API never labels a one- or two-card result as a finished deck.
